...
export default function Header() {
        ...
        >
          <div className="flex items-stretch justify-center bg-[#111] border border-[#222] border-t-0 w-full overflow-x-auto">
            {flavorItems.map((item, i) => (
              ...
              >
                <Link
                  ...
                  className={`flex flex-col items-center gap-2 pt-4 pb-3 px-2 xl:px-3 relative 
                    border-b-4 border-transparent transition-colors duration-200 flavor ${item.bg} h-full`}
                  onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.borderBottomColor = item.color)}
                  onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.borderBottomColor = "transparent")}
                >
                  <div className="text-center">
                    <p className="text-white font-medium text-sm xl:header-small uppercase leading-tight">
                      {t(`flavors.${item.nameKey}`)}</p>
                    <p className="txt-small xl:txt-regular mt-0.5" style={{ color: item.color }}>
                      {t(`flavors.${item.subKey}`)}</p>
                  </div>
            ...
            <Link
              href="#"
              onClick={() => setDropdownOpen(false)}
              className="flex flex-col items-center justify-center min-w-25 xl:min-w-35 px-4 gap-1 text-center group/shop 
              flavor bg-suggested-texture"
            >
  ...
  return (
    <>
      <header className="fixed top-0 inset-x-0 z-40 flex items-center h-16 md:h-20.5 bg-[rgba(5,5,5,0.97)] 
      border-b border-zinc-900 backdrop-blur-sm">

        {/* Logo */}
        <Link href="/" className="logo relative bg-[#171717] py-2 md:py-3 px-4 md:px-7.5 w-36 md:w-55 h-full 
        flex items-center justify-center shrink-0">
          <Image src="/assets/logo.png" alt="Monster Energy" width={220} height={94} 
        ...
        <nav className="hidden lg:flex items-center flex-1 px-2 h-full gap-0.5 overflow-x-auto header-smaller">
          <div ref={dropRef} className="relative h-full flex items-center shrink-0">
            <button ref={buttonRef}
              ...
              className="flex items-center gap-1.5 px-3 xl:px-4 h-full text-white font-semibold uppercase relative 
              whitespace-nowrap text-sm xl:header-smaller">
              {t("nav.energyDrinks")}
              <IoChevronDown className={`size-2.5 transition-transform duration-200 text-accent ${dropdownOpen ? "rotate-180" : ""}`} />
              <span className={`absolute bottom-0 inset-x-0 h-0.5 transition-colors duration-200 z-10 
                ${dropdownOpen ? "bg-accent" : "bg-transparent"}`} />
            </button>
          </div>

          {navLinks.map(({ labelKey, href, highlight }) => (
            <Link key={labelKey} href={href}
              className={`px-2 xl:px-3 h-full flex items-center font-semibold uppercase relative group/navitem 
                whitespace-nowrap shrink-0 text-sm xl:header-smaller transition-colors duration-200 
                ${highlight ? "text-accent" : "text-stone-300 hover:text-white"}`}>
              {t(labelKey)}
              <span className="absolute bottom-0 inset-x-0 h-0.5 opacity-0 group-hover/navitem:opacity-100 
              transition-opacity duration-200 bg-monster" />
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-1 pe-2 md:pe-2.5 ms-auto lg:ms-0 shrink-0">
          {/* Globe */}
          <button onClick={() => setLocationOpen(true)} aria-label={t("location.title")}
          ...
          {/* Pin */}
          <button onClick={() => setStoreOpen(true)} aria-label={t("store.title")}
            className="size-9 md:size-10.5 flex items-center justify-center text-white hover:text-accent transition-colors duration-200">
          ...
          {initializationComplete && isAuthenticated
              ...
              : <Link href="/auth/signin"
                  className="hidden md:flex items-center px-3 py-1.5 txt-small font-semibold uppercase border border-[#78be20] 
                  text-[#78be20] hover:bg-[#78be20] hover:text-black rounded-sm transition-colors duration-200">
                  {t("nav.signin")}
                </Link>}

          {/* Hamburger */}
          <button className="lg:hidden size-9 md:size-10.5 flex items-center justify-center text-white hover:text-accent 
            ...
            <AnimatePresence mode="wait" initial={false}>
              {mobileOpen
                ? <motion.span key="close" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} 
                  exit={{ rotate: 90,  opacity: 0 }} transition={{ duration: 0.15 }}><IoClose className="size-5 md:size-6" /></motion.span>
                : <motion.span key="open"  initial={{ rotate: 90,  opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} 
                  exit={{ rotate: -90, opacity: 0 }} transition={{ duration: 0.15 }}><IoMenu  className="size-5 md:size-6" /></motion.span>
              }
            </AnimatePresence>
          </button>
        </div>
      ...
      <AnimatePresence>
        {mobileOpen && <>
            <motion.div className="fixed inset-0 z-45 bg-black/70 lg:hidden"
            ...
            <motion.nav
              ...
              initial="hidden" animate="visible" exit="exit"
              className="fixed top-16 md:top-20.5 ltr:right-0 rtl:left-0 bottom-0 z-46 w-72 sm:w-80 bg-[#0d0d0d] 
              border-s border-zinc-800 overflow-y-auto lg:hidden flex flex-col">
              <MobileAccordion label={t("nav.energyDrinks")} t={t} onClose={() => setMobileOpen(false)} />

              {navLinks.map(({ labelKey, href, highlight }) => (
                <Link key={labelKey} href={href}
                  onClick={() => setMobileOpen(false)}
                  className={`px-6 py-4 font-semibold uppercase txt-larger border-b border-zinc-800 transition-colors duration-200 
                    ${highlight ? "text-accent" : "text-stone-300 hover:text-white"}`}>
                  {t(labelKey)}
                </Link>
              ))}
              ...
              <div className="mt-auto p-6 flex flex-col gap-3 border-t border-zinc-800">
                <button onClick={() => { setLocationOpen(true); setMobileOpen(false); }}
                ...
                <button onClick={() => { setStoreOpen(true); setMobileOpen(false); }}
                  className="flex items-center gap-3 text-stone-300 hover:text-white transition-colors duration-200 txt-larger font-medium">
                ...
                {initializationComplete && (
                  isAuthenticated
                    ? <Link href="/auth/profile" onClick={() => setMobileOpen(false)}
                        className="flex items-center gap-2 text-[#78be20] txt-larger font-medium">
                        {t("nav.profile")}
                      </Link>
                    : <Link href="/auth/signin" onClick={() => setMobileOpen(false)}
                        className="flex items-center justify-center py-3 border border-[#78be20] text-[#78be20] font-semibold 
                        uppercase txt-small rounded-sm hover:bg-[#78be20] hover:text-black transition-colors duration-200">
                        {t("nav.signin")}
                      </Link>
                )}
              </div>
        ...
        {open && <motion.div
            ...
            <div className="grid grid-cols-2 gap-2 p-4 bg-[#111]">
              {flavorItems.map((item) => (
                <Link key={item.nameKey} href="#" onClick={onClose}
                  className={`flex flex-col items-center gap-1.5 p-3 rounded-sm border border-zinc-800 
                    hover:border-zinc-600 transition-colors duration-200 flavor ${item.bg}`}>
