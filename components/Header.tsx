...
export default function Header() {
  ...
  const itemVariants = {
    ...
    visible: (i: number) => ({
      ...
      transition: { delay: 0.09 + i * 0.07, duration: 0.2, ease: [0, 0, 0.2, 1] as const },
    }),
      ...
      <header className="fixed top-0 inset-x-0 z-40 flex items-center h-16 md:h-20.5 bg-[rgba(5,5,5,0.97)] border-b border-zinc-900 backdrop-blur-sm">
        ...
        <nav className="hidden lg:flex items-center justify-center flex-1 px-2 h-full gap-0.5 overflow-x-auto header-larger">
          ...
          <div ref={dropRef} className="relative h-full flex items-center shrink-0">
            ...
            >
              {t("nav.energyDrinks")}
              <IoChevronDown className={`size-2.5 transition-transform duration-200 text-accent ${dropdownOpen ? "rotate-180" : "rotate-0"}`} />
              <span
            ...
            <div className="grid grid-cols-2 gap-2 p-4 bg-[#111]">
              {flavorItems.map((item) => (
                ...
                >
                  ...
                  <p className="txt-smaller text-center" style={{ color: item.color }}>
                    {t(`flavors.${item.subKey}`)}
                  </p>
                </Link>
              ))}
