...
import { Field, input, ToggleBtn, FormStepper, NavButtons } from "./Helpers";
    ...
    try {
      const res = await fetch("/api/profile/register", {
        ...
        body: JSON.stringify({
          ...
          platformLinks: form.platforms.map((p) => ({
            platform: p,
        ...
        {step === 2 && (
          <motion.div
            ...
            <NavButtons
              ...
              onBack={() => goBack(1)}
              nextLabel={t("next")}
              onNext={() => goForward(3, validateStep2)}
            />
          </motion.div>
        )}
