...
import { CONTENT_TYPES, type ContentType } from "@/lib/data/program";
...
export default function SubmissionForm({ nickname, rank, initialData, onSuccess }: Props) {
  ...
  const CONTENT_TYPES_OPTIONS: { value: ContentType; label: string }[] = [
  ...
  async function handleSubmit(ev: React.FormEvent) {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (isEditMode) { onSuccess(); } else { setSubmitted(true); }
      } else {
    ...
    return (
      <motion.div initial={{ opacity:0, scale:0.96 }} animate={{ opacity:1, scale:1 }}
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-[#b6b6b6] max-w-sm leading-relaxed">{t("successBody")}</p>
        </div>
        <OutlinedParaBtn onClick={() => router.push(`/auth/profile`)} withBorder>
          {t("successCta")}
        </OutlinedParaBtn>
      </motion.div>
    );
  ...
  return (
    <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration: 0.3 }}>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <FileUpload
          label={t("screenshot")} value={form.statsScreenshotUrl}
          onChange={(url) => set("statsScreenshotUrl", url)}
          folder="monster-creators/stats" maxMB={5}
          error={errors.statsScreenshotUrl} required
        />
