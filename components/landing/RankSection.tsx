...
export default function RanksSection() {
  ...
  useEffect(() => {
    if (active === -1) return;
    const id = setInterval(() => setActive((a) => (a + 1) % LANDING_RANKS.length), CYCLE_MS);
  ...
  return (
    <section className="w-full bg-[#050505] py-12 md:py-16 overflow-hidden relative">
      ...
      <div className="text-center mb-8 md:mb-10 px-4">
        ...
        <h2 className="font-display font-black text-white uppercase leading-none mb-6"
      ...
      <div className="hidden md:block relative mx-auto max-w-4xl px-6" style={{ minHeight: "520px" }}>
        {/* Can */}
        <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 flex items-center justify-center"
          ...
          <motion.div initial={{ opacity: 0, scale: 0.85 }} whileInView={{ opacity: 1, scale: 1 }}
            ...
            className="relative w-full" style={{ height: "500px" }}>
            <Image src="/assets/program/monster-ranking.png" alt="Monster Ranks" fill
          ...
          return (
            <AnimatePresence key={rank.id}>
              {isShown && (
                ...
                >
                  <motion.div animate={{ width: isActive ? "48px" : "24px", opacity: isActive ? 1 : 0.4 }}
                    transition={{ duration: 0.3 }} className="h-px shrink-0"
                    style={{ background: rank.color }} />
                  <Link href={`/${locale}/ranks#${rank.id}`} className="no-underline group block">
                    <motion.div
                      animate={{
      ...
      <div className="md:hidden px-4">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.85 }} whileInView={{ opacity: 1, scale: 1 }}
          ...
          className="relative mx-auto mb-6" style={{ width: "160px", height: "260px" }}>
          <Image src="/assets/program/monster-ranking.png" alt="Monster Ranks" fill
            className="object-contain drop-shadow-2xl" priority />
        </motion.div>
            ...
            <motion.div key={rank.id}
              ...
              <Link href={`/${locale}/ranks#${rank.id}`}
                ...
                <div className="flex items-center justify-between mb-2">
                  <p className="font-display font-black uppercase text-xl leading-none"
                    style={{ color: rank.color }}>
                    {locale === "ar" ? rank.nameAr : rank.nameEn}
                  </p>
                  <span className="txt-smaller font-bold px-2 py-0.5 rounded"
