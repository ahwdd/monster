...
function FlavorCard({ item, index }: { item: (typeof flavorItems)[number]; index: number }) {
  const tf = useTranslations("flavors");
    ...
    >
      <Link href={`/flavors/${item.nameKey}`} className="flex flex-col items-center no-underline group">
        ...
        <div className="text-center px-1 sm:px-2 mb-2 sm:mb-3">
          ...
          <p className="txt-smaller sm:txt-regular m-0 leading-tight" style={{ color: item.color }}>
            {tf(item.subKey)}
          </p>
        </div>
