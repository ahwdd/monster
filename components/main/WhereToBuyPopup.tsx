          ...
          <div className="hidden sm:flex flex-1 overflow-hidden min-h-125">
              ...
              {flavors.map((f) => (
                <button
                  ...
                  className={`flex items-center gap-2 md:gap-3 w-full p-2.5 md:p-3 text-start transition-colors border-b border-gray-800 hover:bg-gray-900 border-s-[3px] ${
                    selected.id === f.id ? "bg-[#111111] border-s-[#78be20]" : "border-s-transparent"
                  }`}
                >
                  <div className="relative shrink-0 w-10 md:w-12 h-14 md:h-16 rounded-sm overflow-hidden bg-gray-900">
                    <Image src={f.img} alt={isRTL ? f.nameAr : f.name} fill className="object-contain p-1" />
                  </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-6 text-center border-e border-[#222] bg-[#0f0f0f]">
              <div className="relative mb-4 w-28 md:w-35 h-44 md:h-55">
                <Image src={selected.img} alt={isRTL ? selected.nameAr : selected.name} fill className="object-contain" />
              </div>
            ...
            {/* Col 3 — Finder */}
            <FinderCol t={t} activeTab={activeTab} setActiveTab={setActiveTab} address={address} setAddress={setAddress} />
          </div>
          ...
          <div className="sm:hidden flex flex-col flex-1 overflow-hidden">
            ...
            <AnimatePresence mode="wait" initial={false}>
              ...
              {mobileView === "list" && (
                ...
                >
                  <p className="txt-small text-gray-500 px-4 pt-3 pb-2 shrink-0">{t("selectFlavor")}</p>
                  <div className="overflow-y-auto flex-1">
                    {flavors.map((f) => (
                      ...
                      >
                        ...
                        <span className="txt-regular text-gray-300 leading-tight flex-1">{isRTL ? f.nameAr : f.name}</span>
                        {isRTL
              ...
              {mobileView === "detail" && (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 p-4 border-b border-gray-800">
                    ...
                    <div>
                      <h3 className="text-white font-bold txt-larger uppercase">{isRTL ? selected.nameAr : selected.name}</h3>
                      <p className="txt-small text-gray-400 mt-1">{isRTL ? selected.descAr : selected.desc}</p>
                    </div>
                  </div>

                  {/* Finder — reused */}
                  <FinderCol t={t} activeTab={activeTab} setActiveTab={setActiveTab} address={address} setAddress={setAddress} mobile />
                </motion.div>
              )}
...
function FinderCol({
  ...
  return (
    <div className={`flex flex-col bg-surface ${mobile ? "flex-1" : "w-60 md:w-70 shrink-0"}`}>
      {/* Address */}
        ...
        {(["online", "store"] as const).map((tab) => (
          <button
            ...
            className={`flex-1 py-2 txt-smaller font-bold tracking-widest uppercase transition-colors bg-transparent border-b-2 ${
              activeTab === tab ? "text-white border-[#78be20]" : "text-[#555] border-transparent"
            }`}
          >
