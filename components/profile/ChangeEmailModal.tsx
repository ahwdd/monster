...
import { IoClose, IoArrowForward, IoArrowBack, IoMailOutline } from "react-icons/io5";
...
export default function ChangeEmailModal({ currentEmail, onClose }: Props) {
  const t      = useTranslations("profile");
  const locale = useLocale();
  const isRTL  = locale === "ar";
  const toast  = useToast();
  const { refreshUser, sendEmailLoginOTP } = useAuth();
  ...
  async function handleSend() {
    ...
    if (currentEmail && email.toLowerCase() === currentEmail.toLowerCase()) {
      setError(t("errorSameEmail")); return;
    }

    setLoading(true); setError(null);
    try {
      // Check availability
      const check = await fetch(`/api/users/check-email?email=${encodeURIComponent(email)}`);
      const checkData = await check.json();
      if (checkData.exists) { setError(t("errorEmailTaken")); return; }

      const ok = await sendEmailLoginOTP(email);
  ...
  async function handleVerify() {
    ...
    try {
      ...
      const verifyRes = await fetch("/api/auth/login/email/verify-otp-only", {
      ...
      // 2. Update email in DB using current token
      ...
      // 3. Swap cookie to new token so user stays logged in with new email
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
          <div className="flex items-center gap-3">
            <IoMailOutline className="size-5 text-[#78be20]" />
            <h3 className="header-smaller font-display font-semibold text-white uppercase">
              {t(currentEmail ? "changeEmail" : "addEmail")}
            </h3>
          </div>
        ...
        <div className="p-6 space-y-5">
              ...
              >
                {currentEmail && (
                  <p className="txt-small text-zinc-500">
                    {t("currentEmail")}: <span className="text-white">{currentEmail}</span>
                  </p>
                )}
                <div>
                  <input
            ...
            {step === "otp" && (
              ...
              >
                <p className="txt-small text-zinc-400 text-center">
                  {t("otpSentTo")} <span className="text-white font-medium">{email}</span>
                </p>
          ...
          {/* Actions */}
          <div className={`flex gap-3 pt-1 ${step === "otp" ? "" : ""}`}>
            {step === "otp" && (
            ...
            >
              {loading
                ...
                : <>
                    {step === "email" ? t("sendCode") : t("verifyAndSave")}
